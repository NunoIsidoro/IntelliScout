...
route.post('/', async function(req, res, next) {
  ...
  if(verifyName.length > 0)
    res.send({mensagem: "Equipamento já está registado!"});
  else 
    try {
...
route.put('/edit', async function(req, res, next) {
  try {
    console.log(req.body);
    res.json(await equipament.editEquipment(req.body)); //só envia o primeiro
