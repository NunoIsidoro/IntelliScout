...
async function addEquipament(body){
  ...
  const equipment = await db.query(
    ...
    (name_equipment, quantity_equipment, 
      descr_equipment) values (?, ?, ?);`,
       [body.name, body.quantity, body.descr]);
...
async function deleteEquipament(id){
  ...
  async function editEquipment(body){
    ...
    const equipment = await db.query(
      `update equipment set name_equipment = ?, quantity_equipment = ?, descr_equipment = ? where id_equipment = ?;`,
      [body.name, body.quantity, body.descr, body.id]);
