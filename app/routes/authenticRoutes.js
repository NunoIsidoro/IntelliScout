...
route.post('/register', async function(req, res, next){
  const { gmail, password, role} = req.body;
    try {
      ...
      res.json( await db.query(scriptSQL))
...
route.post( '/login', async function (req, res, next){
  ...
  console.log(req.body)
    try {
        ...
        const scriptSQL = `select email_scout_login, password_scout_login from scout_login where email_scout_login = '` + gmail + `' and strcmp(password_scout_login, '` + password + `') = 0;`
