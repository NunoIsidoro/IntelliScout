...
route.get('/district/:id', async function (req, res, next) {
  try {
    res.json(await activity.getDistrict(req.params.id));
  } catch (err) {
    console.error(`*** Erro: ***\n Não consegue encontrar o distrito da atividade.\n`, err.message);
    res.json([{
      'title': 'Pedimos desculpa, não conseguimos encontrar o distrito da atividade :( ...'
    },
    {
      'message': err.message
    }
